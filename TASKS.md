...
- **Status:** ✅ done — LiveFeed now **mints accounts live** (network grows); needs a small P4 wiring add
- **Right now:** committed `sample_data/` unchanged (800/16 rings; ring-recall 1.0, FP 0, precision 1.0, recall ~0.78). New: **`LiveFeed` mints accounts on demand** so the live network grows nodes, not just edges — new legit accounts open at random **1–10-tick** intervals, and each ring is built from **brand-new** accounts (so rings stay disjoint by construction). Verified: 40 ticks → +200 accounts, 15 separate rings (biggest 10), all schema-valid, 12/12 tests green.
...
- **Notes for the team:** `schemas.py` frozen. P1 deliverables: flagship **kingpin** ring, **dynamic decoys**, and the **LiveFeed** engine (now wired by @P4 🙏).
  - **🔴 @P4 (savvas) — LiveFeed now mints accounts → tiny wiring add needed (`api/main.py` + `app.js`):** `next_batch()` batch shape is now `{accounts:[…new…], transactions, ring, clock}` (added `accounts`). For the new accounts/rings to render in live mode, please: **(1)** in `stream_next()`, before `_recompute()`, add `STATE["dataset"]["accounts"].extend(batch.get("accounts", []))`; **(2)** return the new nodes too, e.g. `"new_nodes": batch.get("accounts", [])`; **(3)** in `liveTick()`, `nodesDS.add(...)` those new nodes **before** the existing `new_edges` filter (your `nodesDS.get(src)&&get(dst)` guard currently drops edges to not-yet-added nodes — graceful, but new rings won't show until nodes are added). No crash without it; just no new nodes. Anti-merge guarantee still holds (rings minted on fresh disjoint accounts; legit traffic stays in the legit pool). Ping me if you'd rather I reshape the batch.
...
- **Right now:** All 3 structural detectors clean + tuned. Latest: richer **structuring score** (count + closeness-to-T + burst tightness, §9 #1) — better ranking, and one structuring hub crossed τ → **recall 0.769→0.775, precision held 1.0**. Earlier: settlement-loop FP fix (FP-rings→0), passthrough ratio+speed scaling. **12 tests pass.**
- **Files I'm touching:** `backend/detect/structural.py`, `backend/tests/test_structural.py`
- **Blockers:** —
- **Notes for the team:** Committed-fixture eval: **ring-recall 1.0, FP-rings 0, precision 1.0, recall 0.775, F1 0.873**. **🔴 Investigated the remaining 9 account-recall misses — they are NOT precision-safe in my lane, recommend we stop chasing them:** all 9 are single-large-deposit collectors / few-counterparty (<5) fan accounts with **no structural signature** (one €30–60k transfer they keep, or distribute to 2–4 dests). I measured a collector/consolidation rule: it catches **1** missed mule but adds **2 legit FPs** (`ACC00100/313`) → breaks precision 1.0. These are ring-context/graph signals = **@Andreas's `network.py` consolidation lever** (you flagged this yourself), and worth it only if the team accepts trading the earned precision. My detectors are at their precision-safe frontier. **🟢 @Andreas:** FP-rings is 0 → your `test_scoring.py` guardrail can assert `==0`.
- **Updated:** 2026-06-13
...
- **Status:** 🔵 in review
- **Right now:** **calmed the graph — no more physics churn.** The vis-network used to run its stabilization *on screen*, so users watched every node fly around and re-settle (chaotic with many nodes / during the live feed). Now `renderGraph` settles the layout **behind a "Laying out network…" overlay**, then **freezes physics and reveals the finished static graph** — the churn is never visible. Live feed: new transfers are added in their **final style (no bright pulse)** with physics frozen, so the layout never drifts as the feed streams (Playwright: node positions identical across 17s of streaming). Also fixed a `duplicate id` throw by skipping live edges already present. Earlier: live feed auto-start + Stop=freeze/Resume=continue; dark mode + grouped notifications inbox; LiveFeed stream wiring + speed control; watchlist, goAML, accounts table, filters, case workflow.
- **Files I'm touching:** `frontend/index.html`, `frontend/app.js`, `frontend/style.css` (this round — pure frontend, no API change)
- **Blockers:** —
- **Notes for the team:** **Frontend-only & additive**; §8 API contract untouched, 12/12 backend tests green, plays nicely beside the §6 freeze/review (Accounts view) + P5 AI. New `#graph-loading` overlay + a `stabilizing` class on `#graph`; `renderGraph` settles then freezes physics (already froze on settle — now it also hides the settling). **⚠️ Still the case: the app lands in LIVE mode by default** — `?live=0` (or any fixture deep-link `?view=`/`?q=`/`?filters=`/`#ring`) opens on the committed fixture instead; `?live=1` forces live. If anyone re-renders the graph mid-stream, live edges already in the DataSet are now skipped (no `duplicate id` crash).
- **Updated:** 2026-06-13
