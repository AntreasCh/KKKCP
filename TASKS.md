...
- _2026-06-14 — P5 (Alexandros): **removed the SAR generator again (per request) + hid the stray "Fit" button on the Accounts view.** Reverted the SAR re-add (`git revert` of `sar.py` + `POST /api/rings/{id}/sar` + the inspector Generate-SAR block + the `/api/accounts/{id}` `rings` field) — the route now 405s through the static mount, no UI references remain. Separately, `setView()` now toggles `#fitbtn` with the graph view (same as the Names toggle), so **Fit shows only in the Visual Review graph** (where it works) and is hidden on the Accounts table (where it did nothing); the graph's Fit button is untouched. 12/12 tests pass; Playwright-verified (Accounts: Fit hidden; graph: Fit visible; inspector: no SAR button). — Alexandros_
