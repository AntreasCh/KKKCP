...
- **Status:** ✅ done — kingpin ring, dynamic decoys, LiveFeed engine + **ring-merge bug fixed**; all green (12/12)
- **Right now:** committed `sample_data/` unchanged (800/16 rings; ring-recall 1.0, FP 0, precision 1.0, recall ~0.78). Just fixed P4's reported **LiveFeed ring-merge bug**: distinct live rings were collapsing into one mega-ring. Repro now passes — 40 ticks → 15 bursts → **15 detected rings, biggest 10 accts** (was 2 rings / 62-acct blob).
...
- **Notes for the team:** `schemas.py` frozen. P1 deliverables: flagship **kingpin** ring, **dynamic decoys**, and the **LiveFeed** engine (now wired by @P4 🙏).
  - **🟢 @P4 (savvas) — LiveFeed ring-merge bug FIXED:** root cause was bursts + legit traffic sharing one account pool → bridged rings → Louvain merged them. Fix: `LiveFeed` now keeps **two disjoint pools** — legit traffic flows only through the legit pool (never touches ring accounts), and each burst consumes a **fresh disjoint slice** of a reserved, mule-profiled ring pool. **Batch shape unchanged** (`{transactions, ring, clock}`) — all accounts exist from `initial()`, so your `/api/stream/*` wiring needs no change. Bumped default pool to 300 accounts (≈14 bursts of headroom; pool exhaustion → legit-only ticks, no crash).
