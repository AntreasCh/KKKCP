...
- **Status:** ✅ done — synced to the **descoped** operator console; P1 data layer compliant, 12/12 green
- **Right now:** post-descope (`6247f56`), P1 owns the data the minimal console runs on: **`generate_dataset`** (the **Generate** button's fresh seeded data — kingpin ring + dynamic hard-negative decoys, every account `status:"active"`) and **`Account.status`** (powers fixed **auto-freeze ≥0.90** + block/ban). Committed `sample_data/` unchanged (ring-recall 1.0, FP 0, precision 1.0, recall ~0.78). Verified after the descope merge: **app boots, `generator.py` clean (no `LiveFeed`/`BURSTS` leftovers), 12/12 tests pass.**
- **Files I'm touching:** `backend/data/generator.py`, `backend/schemas.py`, `sample_data/` (done)
- **Blockers:** —
- **Notes for the team:** `schemas.py` frozen. Live P1 deliverables in the descoped product: `generate_dataset` (kingpin + decoys) and `Account.status`. The **`LiveFeed` streaming engine was removed by the descope** (`6247f56`) with the whole live-feed/stream surface — its earlier P1/P4 wiring + account-minting notes are now obsolete. Not regenerating `sample_data/` (tests assert the committed fixture, DESCOPE §10). Nothing open on P1.
- **Updated:** 2026-06-14
...
- **Status:** 🔵 in review
- **Right now:** **scalable account-focus ("filter by account") — the graph never renders the whole dataset.** New `GET /api/graph/account/{id}?hops=1|2` returns a **bounded ego-network** (account + counterparties + their transfers, capped at max_nodes/max_edges, largest transfers kept) — dataset can be millions of txns, the render stays a few hundred nodes. `focusOnAccount()` shows that slice through the calm overlay→freeze path, with a focus banner (account · owner · *N of TOTAL* transfers · +1 hop · ← Overview), emphasises the focus node, **pauses live**, and filters the **Accounts table** to the ego set. Entry points: a **⊙ Txns** button per Accounts row + **⌘K search**. Playwright: focus→17-node slice of a 475-txn set, table filters to 17, +1 hop 14→65, back restores 123/172, 0 errors. Earlier this stretch: wired live account-minting (network grows nodes, no churn), calmed graph rendering (settle→freeze, no physics churn), live auto-start + freeze/resume, dark mode + grouped notifications.
- **Files I'm touching:** `backend/api/main.py`, `frontend/app.js`, `frontend/index.html`, `frontend/style.css`
- **Blockers:** —
- **Notes for the team:** **Additive** — new `GET /api/graph/account/{id}` (reuses the `/api/graph` node/edge shape); §8 otherwise stable, 12/12 tests green, merges clean with the §6 freeze enforcement in `stream_next`. **This is our scale story:** vis-network can't paint millions of edges, so the graph always renders a bounded slice (rings+flagged by default, or one account's ego-network on focus, or filters) and shows a "N nodes · M total txns" chip. With a real DB the `/api/graph/account` scan becomes an indexed lookup — same API. **App still lands in LIVE mode by default** (`?live=0` / fixture deep-links opt out).
- **Updated:** 2026-06-13
...
- _2026-06-14 — P1 (kiriakos): **synced P1 to the descoped console.** Verified after the descope merge: app boots, `generator.py` is clean (no `LiveFeed`/`BURSTS` leftovers — descope removed the streaming engine), **12/12 tests green**. P1's live deliverables in the minimal product = `generate_dataset` (Generate-button data: kingpin + decoys, `status:"active"`) + `Account.status` (auto-freeze ≥0.90 / block-ban). No code change needed; `sample_data/` left as-is per DESCOPE §10. Nothing open on P1. — kiriakos_
- _2026-06-14 — P5 (Alexandros): **re-added the SAR (Suspicious Activity Report) generator** (descope had cut it) — restores the core "MuleNet writes the compliance paperwork" deliverable from REQUIREMENTS §11a/§16. Restored `backend/ai/sar.py` + re-wired `POST /api/rings/{ring_id}/sar`; hardened the LLM parsing (strips ```json fences, flattens nested dict/list fields → clean narrative) and tightened the prompt to emit plain-text JSON. Surfaced in the **account inspector**: any account in a detected ring shows a **📄 Generate SAR** button (per ring) that drafts the report into a teal report card with a Copy button; `/api/accounts/{id}` now returns the account's `rings`. Additive (new `sar.py`, one route, inspector block in `app.js`/`style.css`); template fallback so it works offline. **Verified live (OpenRouter):** DET_001 → clean formatted SAR (summary/parties/activity/recommended action, €199k, 14 accounts); 12/12 tests pass. — Alexandros_
- _2026-06-14 — P5 (Alexandros): **re-added the "Ask MuleNet" copilot as a polished floating chat widget** (the descope had cut it). Restored `backend/ai/copilot.py` (tool-using agent: `list_rings`/`get_ring`/`get_account`/`trace_path`/`compare_rings`) + re-wired `POST /api/ask`; data shapes still match (ring carries score/patterns/key_accounts) so it works as-is on the descoped pipeline. New UI = a bottom-right FAB → slide-up chat panel: gradient header w/ live provider status dot, user/bot bubbles, **collapsible "🔍 Investigation" trace** showing each tool call+result, typing indicator, suggestion chips, Esc-to-close, mobile-responsive. Purely additive (new `copilot.py`, `/api/ask`, a self-contained chat block in `index.html`/`app.js`/`style.css`) — no existing logic touched. **Playwright-verified live:** opens, "Compare the top two rings" → 4 tool calls (`list_rings`/`get_ring`…) → grounded answer (DET_001 0.904 vs DET_002 0.899), trace expands, 0 console errors. Uses the existing `OPENROUTER_API_KEY` from `.env` (template fallback if absent). — Alexandros_
- _2026-06-14 — P3 (Andreas): **✅ DESCOPE LANDED on `main`** (see `DESCOPE.md`). MuleNet is now a minimal operator console: Accounts table = home, per-account **Visual Review** small graph (`/api/graph/account/{id}?suspicious_only=true`), fixed **>=90% auto-freeze** (no slider). **Removed:** live feed/stream, global graph (`GET /api/graph`), freeze slider, ring-UI, per-ring SAR, goAML, Ask copilot, watchlist screening, playback, cmd-K, filters, dark-mode, eval card; **deleted** `ai/sar.py`, `ai/copilot.py`, `frontend/p2.html`. **Kept:** data gen + Generate, full P2/P3 pipeline (detect_communities + build_rings), P5 account AI-analysis, browse frozen + Freeze/Block/Ban/Clear, review modal. Net **-2358 lines**; app boots, **12/12 tests**, eval green (prec 1.0/recall 0.775/ring-recall 1.0/0 FP). **🔴 EVERYONE: `git pull` now** — `frontend/`, `api/main.py`, `generator.py`, `ai/` changed heavily; if you have local edits to those, expect conflicts (resolve toward `main`). Detection logic (P2/P3) is unchanged. — Andreas_
- _2026-06-14 — P3 (Andreas): **🔴 STARTING THE DESCOPE — see `DESCOPE.md`.** Shrinking MuleNet to a minimal operator console (Accounts panel + per-account **Visual Review** graph + fixed >=90% freeze). Removing: live feed/stream, the global whole-market graph, the freeze slider; cutting rings-UI/SAR/goAML/copilot/watchlist/playback/cmd-K/filters/dark-mode/eval-card/p2.html (keeping detect_communities + Block/Ban). **I'm doing the whole thing solo on branch `descope`** to avoid collisions. 🔴 **@P4 @P5 @P1: please PAUSE pushes to `frontend/`, `api/main.py`, `ai/sar.py`, `ai/copilot.py`, `data/generator.py` until I merge** (heavy deletions — they WILL conflict). `main` stays the safe working demo; I'll only merge when it boots + `pytest` green + demo path verified. Detection (P2/P3) logic unchanged. — Andreas_
- _2026-06-13 — P4 (savvas): **scalable account-focus ("filter by account") — the graph never renders the whole dataset.** New `GET /api/graph/account/{id}?hops=1|2` returns a **bounded ego-network** (account + counterparties + transfers, capped — largest transfers kept), same node/edge shape as `/api/graph`. `focusOnAccount()` renders that slice via the calm overlay→freeze path with a focus banner (account · owner · *N of TOTAL* transfers · **+1 hop** · **← Overview**), emphasises the focus node, **pauses live**, and filters the **Accounts table** to the ego set (with a "show all accounts" clear). Triggered from a per-row **⊙ Txns** button + **⌘K search**. **This is the scale answer:** vis-network can't draw millions of edges, so we always render a bounded slice + a "N nodes · M total txns" chip; with a real DB the account scan becomes an indexed lookup (same API). Additive, §8 stable, merges clean w/ §6 freeze enforcement, 12/12 tests green. **Playwright-verified:** focus→17-node slice of 475 txns, table→17, +1 hop 14→65, back restores 123/172, **0 console errors**. — savvas_
