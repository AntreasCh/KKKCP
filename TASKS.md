...
- **Status:** 🔵 in review
- **Right now:** **live feed now auto-starts on app load + Stop is freeze / Resume continues.** The app opens already polling `/api/stream/next`; **Stop = Pause** (stops polling but keeps the streamed data on screen *and* the server-side `LiveFeed` alive — no `/api/stream/stop`, no fixture restore; banner reads PAUSED); **Resume continues the SAME stream** from exactly where it stopped (network keeps growing, never resets). Leaving the Graph view pauses (resumable); Generate ends the session. Playwright-verified: auto-start, freeze-on-pause (tx count + edges frozen), continue-on-resume (143→143→154, not reset). Earlier: dark mode + grouped notifications inbox; wired P1's LiveFeed stream + speed control, watchlist screening, goAML export, accounts table, filters, case workflow.
- **Files I'm touching:** `frontend/index.html`, `frontend/app.js`, `frontend/style.css` (this round — pure frontend, no API change)
- **Blockers:** —
- **Notes for the team:** **Frontend-only & additive**; §8 API contract untouched, 12/12 backend tests green, plays nicely beside the §6 freeze panel + P5 AI. **⚠️ Heads-up — the app now lands in LIVE mode by default**, so the committed 800-acct fixture is no longer the opening view. Escape hatches kept: `?live=0` opens on the fixture, and fixture deep-links still work (`?view=accounts`, `?q=`, `?filters=1`, `#<ring>` all skip auto-start; `?live=1` forces live). Pause/Resume is frontend-only because Stop no longer calls `/api/stream/stop` — the server `LiveFeed` stays alive between pause and resume, so `next_batch()` just continues. **@P1:** confirmed your two-pool fix holds live — 19 distinct rings, no mega-blob.
- **Updated:** 2026-06-13
