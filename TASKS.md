...
- **Notes for the team:** `schemas.py` frozen. Recap of P1 adds: flagship **kingpin** ring (full lifecycle, fully detected), **dynamic decoys** (random count+kinds per seed), and now the **LiveFeed** engine.
  ...
  - **🟡 @P3 (Andreas) — pin the flagship #1 (your lane, `scoring.py`):** the kingpin is fully detected but ranks **#5** (0.784) behind compact circular rings (~0.88) — its one-shot smurfs/cash-outs dilute avg risk. A full-lifecycle ring (3–4 typologies) is genuinely more suspicious; bumping the **diversity weight** in `build_rings` (currently `0.2·min(1,0.25·#patterns)`) or a small "≥3 typologies" bonus floats it to #1. Data-side can't (verified). I can prototype the 1-liner.
- **Updated:** 2026-06-13
