...
- **Status:** ✅ done — kingpin ring + dynamic decoys + **LiveFeed streaming engine**; all green (11/11)
- **Right now:** committed `sample_data/` unchanged (800/16 rings; ring-recall 1.0, FP 0, precision 1.0, recall 0.78). New this round: a **`LiveFeed` streaming engine** (`backend/data/generator.py`) for the live-mode demo — `initial(~100 tx)` then `next_batch()` emits new transactions over a virtual clock (mostly legit, ~25% a fresh laundering burst). Additive: doesn't touch `generate_dataset`/`sample_data/`/pipeline, so tests stay green.
- **Files I'm touching:** `backend/data/generator.py`, `sample_data/` (done)
- **Blockers:** —
- **Notes for the team:** `schemas.py` frozen. Recap of P1 adds: flagship **kingpin** ring (full lifecycle, fully detected), **dynamic decoys** (random count+kinds per seed), and now the **LiveFeed** engine.
  - **🔴 @P4 (savvas) — wire the live stream (your lane: `api/main.py` + `frontend/`):** engine is ready — `from backend.data.generator import LiveFeed`. Suggested wiring: on live-Generate, `lf = LiveFeed(seed=random); STATE["dataset"],STATE["labels"] = lf.initial(100); STATE["live"]=lf` → returns a ~100-tx network. Add `GET /api/stream/next` → `return STATE["live"].next_batch()` (shape `{transactions:[…], ring:{…}|null, clock}`; append the txns to `STATE["dataset"]`, fire 🚨 on `ring`). Frontend: in live mode **poll `/api/stream/next` every random 1–10s** and render the new edges. Also the requested **live-mode speed-up** lives in your `startLive()` loop (`130ms × 70 batches`) — add a 1×/2×/4× control. Ping me if you want the engine's batch shape tweaked.
  - **~~🟡 @P3 — pin the flagship #1~~ — CANCELLED.** Dropped the kingpin-ranking ask; no action needed from P3. The kingpin stays as a normal fully-detected ring (overlap 1.00); we're not pinning it #1.
- **Updated:** 2026-06-13
...
- **Status:** 🔵 in review
- **Right now:** shipped **dark mode** (topbar 🌙/☀️ toggle, persisted, no-flash bootstrap) + a **grouped live-feed notifications inbox** (🔔 with unread badge): every live 🚨 ring alert is now persisted to localStorage and shown **grouped by laundering pattern**, deduped per ring with an ×N count — so alerts survive the 5s toast and stay reviewable. Verified in-browser (light+dark, persistence across reload). Earlier: wired P1's `LiveFeed` real-time stream + 1×/2×/4× speed control, watchlist screening, goAML export, investigation report, accounts table, filters, case workflow, search.
- **Files I'm touching:** `frontend/index.html`, `frontend/app.js`, `frontend/style.css` (this round — pure frontend, no API change)
- **Blockers:** —
- **Notes for the team:** This round is **frontend-only & additive** — new `#theme-btn`/`#notif-btn` + `#notif-panel`, a `:root[data-theme="dark"]` variable override, and a `recordNotif()`/`renderNotifs()` inbox keyed off the existing `fireRingAlert`. No markup/handlers removed, §8 API contract untouched, P5's AI-analysis intact, 12/12 backend tests still green. **🔴 flagged @P1 a LiveFeed ring-merge bug** (details in Activity Log) — distinct live rings collapse into one mega-ring; root cause is the shared account pool in `generator.py` (his lane), reproduced.
- **Updated:** 2026-06-13
